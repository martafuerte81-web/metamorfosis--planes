...
def generar_pdf(datos, output_path):
    ...
    dias_pref = [d.strip() for d in n.get('diasPreferidos','Lunes,Martes,Jueves,Viernes').split(',') if d.strip()]
    if len(dias_pref) < dias:
        dias_pref = ['Lunes','Martes','Jueves','Viernes','Sabado'][:dias]
    dias_pref = dias_pref[:dias]

    # Construir calendario con dias reales del cliente
    rutinas = []
    for i in range(dias):
        rutinas.append('A' if i % 2 == 0 else 'B')

    cal_headers = [Paragraph(f'<b>{d[:3]}</b>', TH) for d in dias_pref]
    cal_cells = []
    for r in rutinas:
        if r == 'A':
            cal_cells.append(Paragraph('<b>Rutina A</b>', S('tvc', fontName='Helvetica-Bold', fontSize=8, textColor=EM_DARK, alignment=1, leading=12)))
        else:
            cal_cells.append(Paragraph('<b>Rutina B</b>', S('tvc', fontName='Helvetica-Bold', fontSize=8, textColor=EM_DARK, alignment=1, leading=12)))

    cw_sem = [CW/dias]*dias
    sem_t = Table([cal_headers, cal_cells], colWidths=cw_sem)
    sem_ts = list(base_ts())
    ...
    prior_t = Table([[
        Paragraph(f'Nivel de estres: <b>{estres}/5</b> — Prioridad <b>{prioridad}</b>',
                  S('pr', fontName='Helvetica-Bold', fontSize=10, textColor=TEXT_DARK, leading=14)),
